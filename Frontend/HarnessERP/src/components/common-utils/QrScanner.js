...
const QrScanner = () => {
  ...
  const [scannerInstruction, setScannerInstruction] = useState('Place your device over the QR code for scanning');
  ...
  return (
    <View style={styles.container}>
      <QRCodeScanner
        ...
        customMarker={
          <View style={{ justifyContent: 'center', alignItems: 'center', opacity: 0.5 }}>
            <Image source={CustomMarkerImage} style={{ width: moderateScale(150), height: moderateScale(150) }} />
          </View>
        }
        topContent={
          <View style={{ padding: 20, marginBottom: verticalScale(50), top: 0, alignItems: 'center' }}>
            <View style={{ width: '100%', flexDirection: 'row', justifyContent: 'space-between' }}>
              <TouchableOpacity onPress={handleBack}>
                <MaterialIcons name="arrow-back" size={28} color={CustomThemeColors.primary} />
              </TouchableOpacity>
              <Text style={{ fontSize: 20, fontWeight: 'bold', color: CustomThemeColors.primary }}>Scan QR Code</Text>
              <TouchableOpacity onPress={toggleTorch}>
                <MaterialIcons name={torchOn ? 'flash-on' : 'flash-off'} size={28} color={CustomThemeColors.primary} />
              </TouchableOpacity>
            </View>
            <View style={{ width: 'auto' }}>
              <View style={styles.instructionContainer}>
                <MaterialIcons name={instructionIcon} size={20} color="yellow" style={{ opacity: 0.7, paddingHorizontal: 5 }} />
                <Text style={styles.instructionText}>{scannerInstruction}</Text>
              </View>
            </View>
          </View>
        }
        topViewStyle={{ alignItems: 'center', paddingHorizontal: horizontalScale(5), paddingBottom: verticalScale(50), top: 20 }}
        bottomContent={
          <View>
            {showScanAgainButton && (
              <View>
        ...
        bottomViewStyle={{ flexDirection: 'column', paddingTop: verticalScale(40) }}
